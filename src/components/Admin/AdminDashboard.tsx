...
import { FiUsers, FiBook, FiEye, FiBarChart2 } from "react-icons/fi";
...
type ActivePage = "users" | "books" | "view" | "reports";

export default function AdminDashboard() {
  const [active, setActive] = useState<ActivePage>("users");
  ...
  useEffect(() => {
    if (user_id) {
      console.log("Admin ID loaded:", user_id);
    }
  }, [user_id]);
  ...
  const navBtn = (key: ActivePage, icon: React.ReactNode, label: string) => {
    ...
    return (
      ...
      >
        <span style={{ fontSize: 16, width: 20, display: "flex", alignItems: "center" }}>
          {icon}
        </span>
        {label}
        {isActive && (
      ...
      >
        ...
        >
          ...
          >
            ...
            {navBtn("users", <FiUsers />, "Manage Users")}
            {navBtn("books", <FiBook />, "Manage Books")}
            ...
            {navBtn("reports", <FiBarChart2 />, "View Reports")}
          </nav>
